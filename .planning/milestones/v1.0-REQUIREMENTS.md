# v1.0 Requirements (ARCHIVED) — NotesOS Milestone 1 (PARA Notes Sorter)

> Shipped via PRs #1–#6 and **validated end-to-end against real Apple Notes on 2026-06-08**.
> All 44 v1 requirements complete. ~356 tests, ~92% coverage.
...
- [x] **BKUP-01**: System auto-backs-up the Notes database (sqlite + wal + shm) before every write operation by default
- [x] **BKUP-02**: User can create a timestamped, labelled backup on demand
- [x] **BKUP-03**: User can list existing backups
- [x] **BKUP-04**: User can restore from a specific backup or the latest backup
- [x] **BKUP-05**: User can prune old backups; default retention is 10 most recent
- [x] **BKUP-06**: A failed backup aborts the pending write and raises `BackupError`
...
- [x] **TASK-01**: After routing a note, a heuristic scan surfaces action items, named commitments, and inline dates (when enabled)
