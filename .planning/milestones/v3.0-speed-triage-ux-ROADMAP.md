...
2. The before-every-write code path is removed entirely (no `backup_cadence` config); per-session is the sole cadence.
...
3. `U` is repeatable — pressing it again walks further back, unbounded within the session (LIFO to session start).
4. `U` with nothing left to undo is a no-op with a brief hint.
5. The source `FolderPath` is captured at move time so restore targets the true origin.
